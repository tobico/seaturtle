...
export const ButtonBarView = makeClass('ButtonBarView', BaseView, (def) => {
  ...
  def.method('render', function() {
    ...
    jQuery('a', this._element).each(function() {
      const index = jQuery(this).attr('data-index');
      if (jQuery(this).is(`.${ButtonBarView.ALT_BUTTON_MORE_CLASS}`)) {
        makePopup(jQuery(this), self.itemsForAlternatives(self._buttons[index].alternatives));
      } else {
        jQuery(this).click(self._buttons[index].action);
      }
    });
  });
  ...
  def.method('button', function(title, options, action) {
    ...
    this._buttons.push(jQuery.extend({
      title,
      action,
      alternatives: []
    }, options)
  ...
  def.method('buttonElement', function(index) {
    return jQuery(`a[data-index=${index}]`, this._element);
  });
