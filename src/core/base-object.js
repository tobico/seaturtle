...
export const BaseObject = makeClass('BaseObject', null, (def) => {
  ...
  def.classMethod = function(name, fn) {
    if (this[name]) {
      return this[name] = def.OverrideMethod(this[name], fn);
    } else {
      this[name] = fn;
  ...
  def.classMethod('method', function(name, fn) {
    if (fn != null) {
      ...
      return this.prototype[name].toString = def.MethodToString;
    } else {
