...
Spec.describe 'TableView', ->
  beforeEach ->
    ...
    @tableView.columns [new SpecObject(
      field: 'name'
      title: 'Name'
    ), new SpecObject(
      field: 'age'
  ...
  describe '#setColumns', ->
    ...
    it "should set default sort column", ->
      expect(@tableView.sortColumn()).to be(@columns[0])
  ...
  describe '#setSortColumn', ->
    it "should accept an index", ->
      ...
      expect(@tableView.sortColumn()).to be(@tableView.columns()[1])
    
    it "should set sort column", ->
      column = @tableView.columns()[0]
      @tableView.sortColumn column
      expect(@tableView.sortColumn()).to be(column)
