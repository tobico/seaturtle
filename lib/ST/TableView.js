...
ST.class('TableView', 'View', function() {
  this.GroupingEnabled = false;
  this.Instances = [];
  
  this.initializer('withList', function(list) {
    this.init();
    ST.TableView.Instances.push(this);
    this._id = ST.TableView.Instances.length - 1;
    this._rowsByUid = {};
    this._columns = [];
    this._columnsByName = {};
    this._ordered = [];
  ...
  this.property('list');
  this.property('sortColumn');
  this.property('reverseSort');
  this.property('tableClass');
  this.property('tableElement');
  this.property('canCustomizeColumns');
  this.property('lang');
  
  this.destructor(function() {
    ST.TableView.Instances[this._id] = null;
    return this.super();
  });
  
  this.method('setList', function(newList) {
    const self = this;
    if (newList !== this.list) {    
      ...
      if (this._list) {
        this._list.each(function(item) {
          if (!item._uid) { item._uid = ST.Object.UID++; }
  ...
  this.method('setColumns', function(columns, sortColumnIndex, reverseSort) {
  ...
  this.method('sortFunction', function(sortColumn) {
    let column;
    const self = this;
    if (column = sortColumn || this._sortColumn) {
      if (column.sortBy) {
        return ST.makeSortFn(column.sortBy, this._reverseSort);
      } else if (column.sort) {
        ...
        return ST.makeSortFn(item => self.cellValue(item, column)
        , this._reverseSort);
      }
    }
  });

  this.method('setSortColumn', function(sortColumn, reverseSort) {
    const self = this;
  ...
  this.method('rowFor', function(item) {
  ...
  this.method('renderTable', function() {
  ...
  this.method('renderColumnsButton', function() {
    this.element().append('<a class="columnsButton" onmouseover="$(this).addClass(\'columnsButtonHover\')" onmouseout="$(this).removeClass(\'columnsButtonHover\')" href="javascript:;">C</a>');
    return $('.columnsButton', this.element()).popup(this.method('generateColumnsPopup'));
  });
  
  this.method('positionColumnsButton', function() {
    if (this._loaded) {
      const self = this;
      return setTimeout(() => $('.columnsButton', self._element).css('top', self._tableElement.position().top)
      , 1);
    }
  });

  this.method('generateTableHTML', function(html, rows=null, media) {
  ...
  this.method('generateHeaderInnerHTML', function(html, media) {
    if (media == null) { media = 'screen'; }
    html.push('<tr>');
  ...
  this.method('generateColumnHeaderHTML', function(column, html, media) {
    if (media == null) { media = 'screen'; }
    html.push(`<th style="cursor:pointer" onclick="ST.TableView.Instances[${this._id}].setSortColumn(${column.index})">`);
  ...
  this.method('generateBodyInnerHTML', function(html, rows=null, media) {
    if (media == null) { media = 'screen'; }
    const self = this;
  ...
  this.method('activateBody', function() {
    const self = this;
    this._tbody = $('tbody', this._tableElement);
    $('tr', this._tbody).each(function(index) {
      return self._rowsByUid[self._ordered[index]._uid] = this;
    });
    return this._list.each(this.method('activateRow'));
  });
  
  this.method('generateRowHTML', function(item, html, media) {
    if (media == null) { media = 'screen'; }
  ...
  this.method('generateRowInnerHTML', function(item, html, media) {
    if (media == null) { media = 'screen'; }
    return (() => {
      const result = [];
      for (let column of Array.from(this._columns)) {      
  ...
  this.method('activateRow', function(item) {
    const cells = $("td", this._rowsByUid[item._uid]);
    let i = 0;
    return (() => {
      const result = [];
      for (let column of Array.from(this._columns)) {      
  ...
  this.method('cellValue', function(item, column, media) {
    if (media == null) { media = 'screen'; }
    if (column.value) {
  ...
  this.method('refreshTable', function() {
    if (this._loaded) {
      this.refreshHeader();
      return this.refreshBody();
    }
  ...
  this.method('refreshHeader', function() {
    if (this._loaded) {
      const thead = $('thead', this._tableElement);
      const html = [];
  ...
  this.method('refreshBody', function() {
    if (this._loaded) {
      const html = [];
  ...
  this.method('refreshRow', function(item) {
    let row;
  ...
  this.method('toggleColumn', function(column) {
  ...
  this.method('generateColumnsPopup', function() {
    const self = this;
    const a = [];
    for (let column of Array.from(this._columns)) {
  ...
  this.method('titleForColumn', function(column, full) {
  ...
  this.method('listItemAdded', function(list, item) {  
    if (!item._uid) { item._uid = ST.Object.UID++; }
    ...
    if (this._loaded) {
      ...
      this._rowsByUid[item._uid] = $('tr:last-child', this._tbody);
      this.activateRow(item);
      return ST.once(`sort${this._uid}`, this.method('sort'));
    }
  });
  
  this.method('listItemRemoved', function(list, item) {
    let index, row;
  ...
  this.method('listItemChanged', function(list, item) {
    if (this._loaded) {
  ...
  this.method('generatePrintHTML', function(html, options) {
  ...
  this.method('print', function(options) {
    if (options == null) { options = {}; }
    const html = [];
  ...
  this.method('saveColumns', function() {
    if (!this._persistColumnsStorage || !this._persistColumnsKey) { return; }
    const value = {};
    for (let column of Array.from(this._columns)) {
  ...
  return this.method('_headerChanged', function(oldValue, newValue) {
