...
ST.module('Model', function() {
  this._byUuid        = {};
  this._notFound      = {};
  this._generateUUID  = Math.uuid || (function() { if (!this.NextUUID) { this.NextUUID = 0; } return this.NextUUID++; });
  this._storage       = null;
  this._changes       = {};
  ...
  this.recordChange = function(type, uuid, model, data) {    
  ...
  this.sync = function(url, async, additionalData=null) {
  ...
  this.autoSync = function(url, delegate=null, syncBeforeUnload) {
    if (syncBeforeUnload == null) { syncBeforeUnload = false; }
    window.sync = async => ST.Model.sync(url, async);
  ...
  return this.storage = function(newStorage) {
    const self = this;
    if (newStorage != null) {
      ...
      if (newStorage) {
        ...
        return storage.each(function(key, value) {
          if (value && value.model && window[value.model] && !self._byUuid[key]) {
            let model;
            return model = ST.Model.Base.createWithData(value);
          }
        });
      }
    } else {
