...
ST.class('ModelFieldView', 'FieldView', function() {
  // Mappings for event key code to result index
  this.KEY_CODES = { 49:0, 50:1, 51:2, 52:3, 53:4, 54:5, 55:6, 56:7, 57:8, 48:9, 97:0, 98:1, 99:2, 100:3, 101:4, 102:5, 103:6, 104:7, 105:8, 96:9 };
  
  this.RESULT_LIMIT = 5;
  
  // Mappings from result index to label for key
  this.KEY_LABELS = '123456789'.split('');
  
  this.initializer('withModel', function(model) {
    this.init();
    this._model = model;
    this._scope = null;
    this._text = '';
  ...
  this.initializer('withScope', function(scope) {
    this.initWithModel(scope.model());
    return this._scope = scope;
  });
  
  this.property('searching');
  this.property('placeholder');
  this.property('results');
  this.property('selectedResult');
  this.property('acceptsNull');
  this.property('resultListElement');
  this.property('searchRemotelyAt');
  
  this.method('allowCreateWithLabel', function(label) {
  ...
  this.method('setInputValue', function(value) {
    if (value) {
      this._text = ST.trim(value.toFieldText());
  ...
  this.method('render', function() {
    ...
    this._resultListElement = $('<div class="ModelFieldViewResults"></div>');
    this._resultListElement.hide();
    this._resultListElement.mouseout(() => {
      if (!this._hiding) { return this.selectedResult(-1); }
    });
    return $(document.body).append(this._resultListElement);
  });
  ...
  this.method('inputChanged', function() {
    const value = ST.trim(this._inputElement.val());
    if (this._text !== value) {
      this._text = value;
  ...
  this.method('_selectedResultChanged', function(oldValue, newValue) {
    if (this._resultListElement) {
      const rows = $('tr', this._resultListElement);
      if (oldValue >= 0) { rows.eq(oldValue).removeClass('selected'); }
      if (newValue >= 0) { return rows.eq(newValue).addClass('selected'); }
    }
  });
  
  this.method('inputKeyDown', function(event) {
    switch (event.which) {
      case ST.View.VK_ESCAPE:
        this.blur();
      ...
      case ST.View.VK_TAB:
        ...
        break;
      default:
        if (ST.ModelFieldView.KEY_CODES[event.which] != null) {
          const n = ST.ModelFieldView.KEY_CODES[event.which];
  ...
  this.method('performSearch', function(search) {
    const self = this; 
    if (this._searching) {
  ...
  this.method('showResultList', function() {
  ...
  this.method('hideResultList', function() {
    if (this._resultListVisible) {
      this._resultListElement.hide();
  ...
  this.method('showSearchProgress', function() {
    this._resultListElement.html('<table><tr><td>Searching...</td></tr></table>');
    return this.showResultList();
  });
  
  this.method('showResults', function(results) {
    const self = this;
    
    this._results = results;
    if (this._results || this._canCreate) {
      if (this._results.length > ST.ModelFieldView.RESULT_LIMIT) {
        this._results.splice(ST.ModelFieldView.RESULT_LIMIT);
      }
  ...
  this.method('chooseResult', function(result) {
    if (result === 'new') {
  ...
  return this.method('chooseByText', function(text) {
    if (text === 'new') {
