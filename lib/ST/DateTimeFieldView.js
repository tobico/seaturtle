...
ST.class('DateTimeFieldView', 'FieldView', function() {
  this.initializer(function() {
    this.super();
    return this._dateElement = (this._timeElement = null);
  });
  
  this.method('padNumber', function(number) {
    let s = `${number}`;
    if (s.length === 1) { s = `0${s}`; }
    return s;
  });
  
  this.method('isoDate', function(date) {
  ...
  this.method('isoTime', function(time) {
    let result;
  ...
  this.method('dateValue', function(value) {
  ...
  return this.method('timeValue', function(value) {
