STView.subClass('STAutoField', {
    ...
    {
        var self = this;
        this.resultList = ST.tableTag().hide().mouseover(function() {
            self.mouseOverResults = true;
        }).mouseout(function() {
            self.mouseOverResults = false;
        }).addClass('STAutoFieldResults').appendTo(document.body);
    },
    ...
    showResults: function(term, results)
    {
        ...
        self.resultList.css({
            position: 'absolute',
            left: offset.left,
            top: offset.top + self.inputElement.height() + STAutoField.OFFSET,
            width: self.popupWidth || (self.inputElement.width() + STAutoField.OFFSET)
        }).show();
    },
