...
ST.class('WizardView', ST.View, function() {
  this.initializer(function() {
    ...
    this._stepElement = $('<div style="height: 200px"></div>');
  ...
  this.method('addStep', function(stepDefinition) {
  ...
  this.method('dialogButtons', function(dialog, buttonbar) {
    this._dialog = dialog;
  ...
  this.method('renderStep', function(stepDefinition) {
    const self = this;
    ...
    const components = {
      paragraph(text) {
        return $(`<p>${text}</p>`).appendTo(element);
      },
      radioGroup(options) {
        ...
        for (let value in options.options) {
          ...
          (function(value) {
            const input = $(`<input type=\"radio\" name=\"${options.field}\" value=\"${value}\" />`);
            ...
            li.click(function() {
              ...
              return $(this).addClass('radio-item-selected');
            });
      ...
      checkbox(options) {
        if (!data[options.field]) { data[options.field] = options.default; }
        const input = $(`<input type=\"checkbox\" id=\"${options.field}\" />`);
      ...
      textbox(options) {
        if (!data[options.field]) { data[options.field] = options.default; }
        const input = $(`<textarea id=\"${options.field}\" />`);
        input.css({
          display:  'block',
          width:    '450px',
          height:   '60px',
  ...
  this.method('getHeight', function() {
    return this._stepElement.css('height');
  });
  
  this.method('setHeight', function(value) {
    return this._stepElement.css('height', value);
  });
  
  this.method('updateButtons', function() {
    if (this._buttonBar) {
  ...
  this.method('_stepIndexChanged', function(oldValue, newValue) {
    if (this._loaded) {
  ...
  this.method('atFirstStep', function() {
    return this._stepIndex === 0;
  });
  
  this.method('atLastStep', function() {
    return this._stepIndex === (this._steps.length - 1);
  });
  
  this.method('nextStep', function() {
    if (this.atLastStep()) {
      return this.finish();
    } else {
  ...
  return this.method('present', function(title) {
    return ST.DialogView.createWithTitleView(title, this);
  });
});