STObject.subClass('STView', {
    ...
    init: function()
    {
        this.initWithElement(ST.customTag('view').addClass(this.$._name));
    },
    ...
    {
        if (this.loaded) {
            if (this.header) this.header.getElement().detach();
            if (this.footer) this.footer.getElement().detach();
            this.unload();
        }
        this.load();
    },
