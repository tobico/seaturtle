...
ST.module('Model', function() {
  return this.class('Scope', 'List', function() {
    ...
    this.property('model', 'read');
  
    this.method('fork', function(block) {
      const scope = this._class.createWithScope(this);
      if (block) { block.call(scope); }
      return scope;
    });
    
    this.method('where', function(...conditions) {
    ...
    this.method('order', function(...orders) {
      return this.fork(function() {
    ...
    this.method('addBindings', function() {
    ...
    this.method('removeBindings', function() {
      if (this._bindingsAdded) {
    ...
    this.method('bind', function(trigger, receiver, selector) {
      this.populate();
      this.addBindings();
      return this.super(trigger, receiver, selector);
    });
  
    this.method('unbindAll', function(receiver) {
      this.super(receiver);
      if (!this.isBound()) { return this.removeBindings(); }
    });

    this.method('each', function(callback) {
      this.populate();
    ...
    this.method('forgetAll', function(destroy) {
    ...
    this.method('destroyAll', function() {
      return this.forgetAll(true);
    });
    
    this.method('build', function(data) {
      const defaults = {};
      for (let condition of Array.from(this._conditions)) {
    ...
    this.method('search', function(keywords, limit) {
    ...
    this.method('targetItemAdded', function(target, item) {
      if (item.matches(this._conditions)) { this.add(item); }
      return this.sort();
    });

    return this.method('targetItemRemoved', function(target, item) {  
