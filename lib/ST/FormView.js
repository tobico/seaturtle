...
import { EnumFieldView } from './EnumFieldView'
import { BoolFieldView } from './BoolFieldView'
import { DateTimeFieldView } from './DateTimeFieldView'
import { ModelFieldView } from './ModelFieldView'
import { Command } from '../util/command'
import { detectMac } from '../util/detect'

export const FormView = makeClass(View, (def) => {
  ...
  def.initializer(function(options, definition) {
    ...
    const dsl = {
      ...
      text(attribute, options) {
        if (options == null) { options = {}; }
        return this._add(TextFieldView.create(), attribute, options);
      },
      enum(attribute, options) {
        if (options == null) { options = {}; }
        $.extend(options, self.detailsFor(attribute));
        return this._add(EnumFieldView.createWithValuesNull(options.values, options.null), attribute, options);
      },
      bool(attribute, options) {
        if (options == null) { options = {}; }
        return this._add(BoolFieldView.create(), attribute, options);
      },
      datetime(attribute, options) {
        if (options == null) { options = {}; }
        return this._add(DateTimeFieldView.create(), attribute, options);
      },
      model(attribute, options) {
        if (options == null) { options = {}; }
        const details = self.detailsFor(attribute);
        const field = ModelFieldView.createWithModel(self._model._namespace.class(details.model));
